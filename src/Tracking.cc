...
namespace ORB_SLAM3
{
    ...
    Sophus::SE3f Tracking::GrabKpMonocular(const std::vector<cv::KeyPoint> &keys, const cv::Mat &descriptors, const double &timestamp, string filename)
    {
        if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET || (lastID - initID) < mMaxFrames)
            mCurrentFrame = Frame(keys, descriptors, timestamp, mpIniORBextractor, mpORBVocabulary, mpCamera, mDistCoef, mbf, mThDepth);
        else
            mCurrentFrame = Frame(keys, descriptors, timestamp, mpORBextractorLeft, mpORBVocabulary, mpCamera, mDistCoef, mbf, mThDepth);
    ...
    void Tracking::MonocularInitialization()
    {
        ...
        std::cout << "ready to initialize: " << mbReadyToInitializate << endl;

        if (!mbReadyToInitializate)
        {
            std::cout << "\tif\n";
            std::cout << "\tsize: " << mCurrentFrame.mvKeys.size() << std::endl;
            // Set Reference Frame
            if (mCurrentFrame.mvKeys.size() > 100)
            {
                std::cout << "\t\tinizializzazione\n";
        ...
        else
        {
            ...
            if (((int)mCurrentFrame.mvKeys.size() <= 100) || ((mSensor == System::IMU_MONOCULAR) && (mLastFrame.mTimeStamp - mInitialFrame.mTimeStamp > 1.0)))
            {
                ...
                std::cout << "\t\tless than 100 features: exit\n";
            ...
            std::cout << nmatches << " matches\n";
            // Check if there are enough correspondences
            if (nmatches < 20)
            {
                mbReadyToInitializate = false;
                std::cout << "\t\tless than 50 mathces: exit\n";
            ...
            bool res = mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn, mCurrentFrame.mvKeysUn, mvIniMatches, Tcw, mvIniP3D, vbTriangulated);
            std::cout << "\tReconstructWithTwoViews: " << res << endl;
