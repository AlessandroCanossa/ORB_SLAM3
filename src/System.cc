...
namespace ORB_SLAM3
{
    ...
    Sophus::SE3f System::TrackKpMonocular(const std::vector<cv::KeyPoint> &kpts, const cv::Mat &desc, const double &timestamp, const vector<IMU::Point> &vImuMeas, string filename)
    {
        {
            unique_lock<mutex> lock(mMutexReset);
            if (mbShutDown)
        ...
        // cout << "grab monocular\n";
        Sophus::SE3f Tcw = mpTracker->GrabKpMonocular(kpts, desc, timestamp, filename);
